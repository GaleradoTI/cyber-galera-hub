...
export function PartnersCarousel() {
  ...
  const normalizeLinks = (value: unknown): { label: string; url: string }[] => {
    if (Array.isArray(value)) return value as { label: string; url: string }[];
    return Object.entries((value as Record<string, string>) ?? {}).map(([label, url]) => ({ label, url: String(url ?? "") }));
  };
  ...
  return (
    <section className="container mx-auto px-4 py-16">
      ...
      >
        <CarouselContent>
          {partners.map((p: any) => {
            const links = normalizeLinks(p.social_links).filter((l) => l.url?.trim());
            return (
              <CarouselItem key={p.id} className="basis-1/2 md:basis-1/3 lg:basis-1/4">
                <div className="glass rounded-xl p-5 h-full flex flex-col items-center text-center hover-glow-cyan transition border border-border/40">
                  ...
                  {p.website_url && (
                    <a href={p.website_url} target="_blank" rel="noopener noreferrer" className="mt-2 text-[10px] text-primary inline-flex items-center gap-1 hover:underline">
